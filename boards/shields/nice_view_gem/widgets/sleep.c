...
void draw_sleep_screen(lv_obj_t *canvas) {
    ...
    // Center the 96px icon on the 168px wide landscape canvas: (168-96)/2 = 36
    lv_canvas_draw_img(canvas, 36, 12, &sleep_icon, &img_dsc);

    // Show "SLEEP" centered below the icon
    lv_draw_label_dsc_t label_dsc;
    init_label_dsc(&label_dsc, LVGL_FOREGROUND, &quinquefive_8, LV_TEXT_ALIGN_CENTER);
    lv_canvas_draw_text(canvas, 0, 118, LANDSCAPE_W, &label_dsc, "SLEEP");
}
