...
static void draw_level_peripheral(lv_obj_t *canvas, const struct status_state *state) {
    ...
    if (level > 90) {
        lv_canvas_draw_img(canvas, 102, 6, &r_battery_100, &img_dsc_r);
    } else if (level > 75) {
        lv_canvas_draw_img(canvas, 102, 6, &r_battery_90, &img_dsc_r);
    } else if (level > 50) {
        lv_canvas_draw_img(canvas, 102, 6, &r_battery_75, &img_dsc_r);
    } else if (level > 25) {
        lv_canvas_draw_img(canvas, 102, 6, &r_battery_50, &img_dsc_r);
    } else if (level > 10) {
        lv_canvas_draw_img(canvas, 102, 6, &r_battery_25, &img_dsc_r);
    } else if (level > 1) {
        lv_canvas_draw_img(canvas, 102, 6, &r_battery_10, &img_dsc_r);
    }
}
